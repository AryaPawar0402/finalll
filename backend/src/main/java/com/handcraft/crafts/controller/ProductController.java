...
public class ProductController {
    ...
    // ✅ NEW: Get all products with shop details (for all authenticated users)
    @GetMapping("/all-with-shops")
    public ResponseEntity<List<ProductWithShopDTO>> getAllProductsWithShops() {
        List<ProductWithShopDTO> productsWithShops = productService.getAllProductsWithShops();
        return ResponseEntity.ok(productsWithShops);
    }
}
